...
RSpec.describe 'Posts', type: :request do
  user1 = User.create!(email: 'user@example.com', password: 'foobar', password_confirmation: 'foobar')
  post1 = user1.posts.create!(title: 'title', content: 'A new post')

  let(:user_params) do
    {
      user: { email: 'user@example.com', password: 'foobar' }
    }
  end

  let(:attributes) do
    {
      post: { title: 'title', content: post1.content }
    }
  end
